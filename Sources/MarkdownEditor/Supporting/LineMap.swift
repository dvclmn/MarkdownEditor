...
struct LineMap {
    ...
    struct LineInfo {
        ...
        // Add any other Markdown-specific line information here
    }
    ...
    enum LineType {
        ...
        case blockQuote
        case codeBlock
        // Add other Markdown-specific line types as needed
    }

    private var lines: [Line] = []

    init(string: String) {
        updateLines(for: string)
    }

    mutating func updateLines(for string: String) {
        lines = []
        ...
        while currentIndex < nsString.length {
            ...
            let lineType = determineLineType(lineString)
            lines.append(Line(range: lineRange, info: LineInfo(lineType: lineType)))
            currentIndex = lineRange.upperBound
        }
    }

    private func determineLineType(_ line: String) -> LineType {
        // Implement Markdown-specific line type detection here
        // This is a simplified example:
        if line.hasPrefix("#") { return .header(level: line.prefix(while: { $0 == "#" }).count) }
        if line.hasPrefix(">") { return .blockQuote }
        if line.hasPrefix("    ") || line.hasPrefix("\t") { return .codeBlock }
        if line.hasPrefix("- ") || line.hasPrefix("* ") || line.hasPrefix("+ ") { return .listItem }
        return .normal
    }

    func lineContaining(index: Int) -> Int? {
        lines.firstIndex(where: { $0.range.contains(index) })
    }

    func linesAffected(by editedRange: NSRange, changeInLength delta: Int) -> Range<Int> {
        let startLine = lineContaining(index: editedRange.lowerBound) ?? 0
        let endLine = lineContaining(index: editedRange.upperBound + delta) ?? (lines.count - 1)
        return startLine..<(endLine + 1)  // Use '..<' and add 1 to endLine to include it
    }
...
//        let startLine = lineContaining(index: editedRange.lowerBound) ?? 0
//        let endLine = lineContaining(index: editedRange.upperBound + delta) ?? (lines.count - 1)
//        return Range(startLine...endLine)  // Convert ClosedRange to Range
//    }

    // Add other necessary methods here, simplified for Markdown use
}
