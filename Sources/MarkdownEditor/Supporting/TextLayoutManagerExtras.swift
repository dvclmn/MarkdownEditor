...
extension NSTextLayoutManager {
    ...
    ///
    /// - Parameters:
    ///   - markdownTextViewDelegate: The code view delegate whose `textDidChange` hook ought to be used for setting attributes.
    ...
    func setSafeRenderingAttributesValidator(
        ...
        _ renderingAttributesValidator: @escaping (NSTextLayoutManager, NSTextLayoutFragment) -> Void) -> NSKeyValueObservation? {
            ...
            let currentTextDidChange = markdownTextViewDelegate.textDidChange
            markdownTextViewDelegate.textDidChange = { textView in
