...
//    drawCodeBlockBackgrounds(in: charRange, textStorage: textStorage, textContainer: textContainer, origin: origin)
    drawMergedCodeBlockBackgrounds(in: charRange, textStorage: textStorage, textContainer: textContainer, origin: origin)
  ...
  private func drawMergedCodeBlockBackgrounds(in charRange: NSRange, textStorage: NSTextStorage, textContainer: NSTextContainer, origin: NSPoint) {
    ...
    textStorage.enumerateAttribute(CodeBackground.codeBlock.attributeKey, in: charRange, options: []) { (value, range, _) in
      ...
      if let existing = currentRange {
        ...
        if range.location <= existing.upperBound {
          currentRange = NSRange(location: existing.location,
    ...
    for range in mergedRanges {
      ...
      if let rect = unionRect {
        let drawRect = rect.offsetBy(dx: origin.x, dy: origin.y)
          .insetBy(dx: -4, dy: -2) // Slightly larger padding
        
        let roundedPath = NSBezierPath(roundedRect: drawRect,
